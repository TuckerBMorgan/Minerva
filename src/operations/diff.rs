...
pub fn compile_diff_operation(inputs: Vec<MemoryToken>, output_variable: MemoryToken) -> Vec<JobType> {
    ...
    if output_variable.size * 10 > cpus {
        return vec![
            JobType::new_diff_type(inputs[0].start, inputs[1].start,  output_variable.start, output_variable.size)
        ];
    }
    let mut jobs = vec![];
    for cpu in 0..cpus {
        ...
        let job = JobType::new_diff_type(left_hand_start, right_hand_start,  output_variable.start + chunk_start, operations);
        jobs.push(job);
    }
...
pub fn preform_diff_job(lhs_start: usize,  rhs_start: usize, destination_start: usize, length: usize, memory_pointer: &mut JobPtr) {
    unsafe {
