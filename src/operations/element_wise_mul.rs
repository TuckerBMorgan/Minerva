...
pub fn compile_element_wise_mul_job(inputs: Vec<MemoryToken>, output_variable: MemoryToken) -> Vec<JobType> {
    ...
    if output_variable.size * 10 > cpus {
        return vec![
            JobType::new_element_wise_mul_type(inputs[0].start, inputs[1].start,  output_variable.start, output_variable.size)
        ];
    }
    let mut jobs = vec![];
    for cpu in 0..cpus {
        ...
        let job = JobType::new_element_wise_mul_type(left_hand_start, right_hand_start,  output_variable.start + chunk_start, operations);
        jobs.push(job);
    }
