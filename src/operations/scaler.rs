...
pub fn compile_scalar_operation(inputs: Vec<MemoryToken>, output_variable: MemoryToken) -> Vec<JobType> {
    ...
    if output_variable.size * 10 > cpus {
        return vec![
            JobType::new_scaler_type(inputs[0].start, inputs[1].start,  output_variable.start, output_variable.size)
        ];
    }
    let mut jobs = vec![];
    for cpu in 0..cpus {
        ...
        let right_hand_start = inputs[1].start;
        let operations;
        if cpu == cpus - 1 {
        ...
        let job = JobType::new_scaler_type(left_hand_start, right_hand_start,  output_variable.start + chunk_start, operations);
        jobs.push(job);
    }
...
pub fn preform_scaler_operation(lhs_start: usize,  rhs_start: usize, destination_start: usize, length: usize, memory_pointer: &mut JobPtr) {
    unsafe {
        let memory =  memory_pointer.memory_ptr;
        for i in 0..length {
            let memory_offset = destination_start + i;
            let left_offset = lhs_start + i;
            let right_offset = rhs_start;
