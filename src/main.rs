...
impl AddJobEx {
    pub fn new(lhs_start: usize, rhs_start: usize, destination_start: usize, length: usize) -> AddJobEx {
        AddJobEx {
            lhs_start,
            rhs_start,
            destination_start,
...
fn imperative(matrix_size: usize) -> i64 {
    let mut vectors = vec![];
    for i in 0..3 {
        let mut new_one = vec![];
...
pub fn bench_imperative() {
    ...
    for i in 0..25 {
        let result = imperative(matrix_size);
...
fn jank(matrix_size: usize) -> i64 {
    ...
    for cpu in 0..cpus {
        ...
        let job = AddJobEx::new(left_hand_start, right_hand_start, number_of_operations * cpu, operations);
        jobs.push(job);
    }
